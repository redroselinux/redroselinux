...
bare-build: installer initramfs iso
no-clean: installer initramfs iso vm
...
debug-mkfile:
	python3 src/debug-mode/start.py

stop-debug-mkfile:
	python3 src/debug-mode/stop.py

.PHONY: all initramfs iso clean vms installer run-installer clean-downloads clean-all bare-build no-clean vm help debug-mkfile stop-debug-mkfile installed-vm